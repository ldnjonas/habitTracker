...
export function insightRouten(app: FastifyInstance, db: Db): void {
  ...
  app.get("/stats/summary", async (anfrage) => {
    ...
    const entries = await store.listEntries(db, von, tag);
    const exceptions = await store.listExceptions(db, von, tag);
  ...
  app.get("/stats/overview", async (anfrage) => {
    ...
    const summaries = overview(
      habits, await store.listEntries(db, from, to), await store.listExceptions(db, from, to),
      from, to, today);
...
async function arbeitsreichsterTag(
  db: Db, habits: Awaited<ReturnType<typeof store.listHabits>>,
  bis: CalendarDate, today: CalendarDate,
): Promise<number> {
  const von = addMonths(bis, -12);
  const summaries = overview(
    habits, await store.listEntries(db, von, bis), await store.listExceptions(db, von, bis),
    von, bis, today);
  let hoechste = 0;
